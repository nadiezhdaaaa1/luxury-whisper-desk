    ...
    <section id="categories" className="py-20 lg:py-28 bg-surface/60 border-y border-hairline">
      <div className="container-page">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {cats.map((c, i) => (
            <div key={c.title} className={`bg-white border border-hairline rounded-2xl shadow-soft p-6 flex flex-col gap-3 relative overflow-hidden ${i >= cats.length - 2 ? "opacity-60" : ""}`}>
              {c.image && (
                <img
                  src={c.image}
                  alt={c.title}
