// V2 aha reveal + account creation/login — self-contained.
// Full journey handoff for V2 happens here: after auth we call the V2
...
export function AhaRevealV2({ answers, email, onBack }: Props) {
  ...
  useEffect(() => {
    track("aha_reveal_v2", { brands: answers.brands.length });
  ...
  const resolveTier = useMemo(() => {
    ...
    return (name: string, cat: CategoryV2 | null) => {
      const row = list.find(
  ...
  const range = useMemo(
    () => indicativeRangeV2(answers.brands, answers.segments, answers.categories, resolveTier),
    [answers.brands, answers.segments, answers.categories, resolveTier],
  );
  const personal = useMemo(
    () => personalizationLineV2(answers.brands, answers.segments, answers.categories),
    [answers.brands, answers.segments, answers.categories],
  );

  // Attempt to persist V2 answers into the profile after auth. Runs once we
  // have a session. If it fails, /app will surface it via its own handoff.
  async function persistAndGoToApp() {
    try {
      await saveQuizAnswersV2({
        data: {
          segments: answers.segments,
          categories: answers.categories,
          brands: answers.brands,
          role: answers.role as RoleV2,
        },
      });
      clearDraftV2();
      track("quiz_v2_completed_saved", { mode: "landing" });
    } catch (e) {
      // Non-fatal: still land in-app; user can retry there.
      console.error("[v2] saveQuizAnswersV2 failed:", e);
    }
  ...
  async function googleSignup() {
    ...
    track("account_created", { method: "google", variant: "v2" });
  ...
  async function sendCode() {
    ...
    setCodeSent(true);
    setCooldown(30);
    track("otp_code_sent", { variant: "v2" });
  }
  ...
  async function verifyCode(e: React.FormEvent) {
    ...
    if (err) {
      setBusy(null);
      track("otp_verify_failed", { message: err.message, variant: "v2" });
      setError(friendlyOtpError(err.message));
      return;
    }
    track("otp_verified", { variant: "v2" });
    track("account_created", { method: "email_otp", variant: "v2" });
    await persistAndGoToApp();
  }
    ...
    <div className="min-h-[100dvh] flex flex-col bg-background text-foreground">
      ...
      <div className="flex-1 mx-auto w-full max-w-3xl px-2 pt-5 pb-8 sm:pt-9 sm:pb-12">
        <div className="min-h-[420px] px-3 sm:px-4">
          <div>
            <span className="eyebrow">Your preview · V2</span>
...
function HeroValueCard({
  ...
  range: ReturnType<typeof indicativeRangeV2>;
  personal: string;
  brandsCount: number;
}) {
  ...
  const catEntries = Object.entries(range.perCategory) as [
    keyof typeof CATEGORY_LABELS_V2,
      ...
      <div className="grid gap-6 sm:gap-8 md:grid-cols-[1.15fr_0.85fr]">
        <div>
          ...
          <div className="mt-2 font-display font-semibold tracking-tight text-primary text-4xl sm:text-5xl leading-[1.05]">
            <span>{formatCompactUSDV2(lowAnim)}</span>
            <span className="mx-2 text-primary/40 font-normal">–</span>
            <span>{formatCompactUSDV2(highAnim)}</span>
          </div>
          <div className="mt-3">
            <div
              ...
              <div className="space-y-1.5 text-sm">
                {catEntries.map(([cat, v]) => (
                  <div key={cat} className="flex items-center justify-between">
                    <span className="text-foreground/75">{CATEGORY_LABELS_V2[cat]}</span>
                    <span className="font-display text-foreground/90 tabular-nums">
                      {formatCompactUSDV2(v.low)}
                      <span className="text-muted-foreground mx-1">–</span>
                      {formatCompactUSDV2(v.high)}
                    </span>
                  </div>
                ))}
              </div>
